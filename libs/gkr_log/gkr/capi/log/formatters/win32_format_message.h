...
inline int gkr_log_win32_format_message(void* channel, long msg_id, int severity, int facility, const char* prefix)
{
    ...
    unsigned cch;
    unsigned len = 0;
    if(!gkr_log_custom_message_start(channel, severity, &buf, &cch)) return 0;
    ...
    if(prefix != NULL)
    {
        ...
        *(size_t*)&arg = (size_t)(unsigned long)msg_id;
        ...
        if(len == 0) return gkr_log_custom_message_cancel(channel);
    }
    ...
    return gkr_log_custom_message_finish(channel, severity, facility);
}
inline int gkr_log_win32_format_message_ex(void* channel, long msg_id, const char* func, const char* file, unsigned line, int severity, int facility, const char* prefix)
{
    ...
    return gkr_log_custom_message_finish_ex(channel, func, file, line, severity, facility);
}
#ifdef __cplusplus
#ifdef __clang__
