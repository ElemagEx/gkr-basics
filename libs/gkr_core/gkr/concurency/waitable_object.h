...
static constexpr std::size_t WAIT_MAX_OBJECTS = sizeof(wait_result_t) * CHAR_BIT - 1;
...
public:
    ...
    static bool wait_many_for(const std::chrono::duration<Rep, Period>& timeout_duration, Objects&&... objects)
    {
        const long long timeout_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(timeout_duration).count();
    ...
    static bool wait_many_until(const std::chrono::time_point<Clock, Duration>& timeout_time, Objects&&... objects)
    {
        const long long timeout_ns = std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - timeout_time).count();
...
public:
    GKR_INNER_API static waitable_object& null() noexcept;
};
