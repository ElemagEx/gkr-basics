...
#define LOG(severity, facility, message)    gkr_log_simple_message(LOG_INSTANCE, severity            , facility, message)

#define LOGF(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, message)
#define LOGE(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, message)
#define LOGW(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, message)
#define LOGI(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, message)
#define LOGV(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, message)
#define LOGD(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, message)
#define LOGT(facility, message)             gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, message)

#define LOG_FATAL(  facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, message)
#define LOG_ERROR(  facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, message)
#define LOG_WARNING(facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, message)
#define LOG_INFO(   facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, message)
#define LOG_VERBOSE(facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, message)
#define LOG_DEBUG(  facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, message)
#define LOG_TRACE(  facility, message)      gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, message)

#define LOG_IF(condition, severity, facility, message)  ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, severity            , facility, message))

#define LOGF_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, message))
#define LOGE_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, message))
#define LOGW_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, message))
#define LOGI_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, message))
#define LOGV_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, message))
#define LOGD_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, message))
#define LOGT_IF(condition, facility, message)           ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, message))

#define LOG_FATAL_IF(  condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, message))
#define LOG_ERROR_IF(  condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, message))
#define LOG_WARNING_IF(condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, message))
#define LOG_INFO_IF(   condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, message))
#define LOG_VERBOSE_IF(condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, message))
#define LOG_DEBUG_IF(  condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, message))
#define LOG_TRACE_IF(  condition, facility, message)    ((!(condition)) ? 0 : gkr_log_simple_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, message))
...
#define LOG_(severity, facility, format, ...)   gkr_log_printf_message(LOG_INSTANCE, severity            , facility, format, __VA_ARGS__)

#define LOGF_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, format, __VA_ARGS__)
#define LOGE_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, format, __VA_ARGS__)
#define LOGW_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, format, __VA_ARGS__)
#define LOGI_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, format, __VA_ARGS__)
#define LOGV_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, format, __VA_ARGS__)
#define LOGD_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, format, __VA_ARGS__)
#define LOGT_(facility, format, ...)            gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, format, __VA_ARGS__)

#define LOG_FATAL_(  facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, format, __VA_ARGS__)
#define LOG_ERROR_(  facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, format, __VA_ARGS__)
#define LOG_WARNING_(facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, format, __VA_ARGS__)
#define LOG_INFO_(   facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, format, __VA_ARGS__)
#define LOG_VERBOSE_(facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, format, __VA_ARGS__)
#define LOG_DEBUG_(  facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, format, __VA_ARGS__)
#define LOG_TRACE_(  facility, message, ...)    gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, format, __VA_ARGS__)

#define LOG_IF_(condition, severity, facility, format, ...) ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, severity            , facility, format, __VA_ARGS__));

#define LOGF_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, format, __VA_ARGS__))
#define LOGE_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, format, __VA_ARGS__))
#define LOGW_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, format, __VA_ARGS__))
#define LOGI_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, format, __VA_ARGS__))
#define LOGV_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, format, __VA_ARGS__))
#define LOGD_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, format, __VA_ARGS__))
#define LOGT_IF_(condition, facility, format, ...)          ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, format, __VA_ARGS__))

#define LOG_FATAL_IF_(  condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_FATAL  , facility, format, __VA_ARGS__))
#define LOG_ERROR_IF_(  condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_ERROR  , facility, format, __VA_ARGS__))
#define LOG_WARNING_IF_(condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_WARNING, facility, format, __VA_ARGS__))
#define LOG_INFO_IF_(   condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_INFO   , facility, format, __VA_ARGS__))
#define LOG_VERBOSE_IF_(condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_VERBOSE, facility, format, __VA_ARGS__))
#define LOG_DEBUG_IF_(  condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_DEBUG  , facility, format, __VA_ARGS__))
#define LOG_TRACE_IF_(  condition, facility, format, ...)   ((!(condition)) ? 0 : gkr_log_printf_message(LOG_INSTANCE, LOG_SEVERITY_TRACE  , facility, format, __VA_ARGS__))
